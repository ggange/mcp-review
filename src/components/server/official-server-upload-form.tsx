...
interface OfficialServerUploadFormProps {
  onSuccess?: () => void
}

export function OfficialServerUploadForm({ onSuccess }: OfficialServerUploadFormProps = {}) {
  ...
  const uploadIcon = async (): Promise<string | null> => {
    if (!iconFile || !formData.name.trim() || !formData.organization.trim()) {
      return null
    }
    ...
    try {
      const serverId = `${formData.organization.trim()}/${formData.name.trim()}`
...
`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.description.trim() || !formData.organization.trim()) {
      setError('Name, organization, and description are required')
      return
    }
    ...
    startTransition(async () => {
      try {
        ...
        // Submit official server
        const response = await fetch('/api/servers/official', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (!response.ok) {
          const data = await response.json()
          throw new Error(data.error?.message || 'Failed to upload official server')
        }
        ...
        setError(err instanceof Error ? err.message : 'Failed to upload official server')
      }
    })
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-4">
        <Badge variant="outline" className="border-amber-500 text-amber-700 dark:border-amber-400 dark:text-amber-300 bg-amber-50 dark:bg-amber-950">
          Official Server
        </Badge>
        <p className="text-sm text-muted-foreground">
          Upload an official server representing an organization
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {success && (
          <p className="text-sm text-emerald-600 dark:text-emerald-400">
            Official server uploaded successfully!
          </p>
        )}
        ...
        >
          {isPending || isUploadingIcon ? (
            <span className="flex items-center gap-2">
              ...
              {isUploadingIcon ? 'Uploading icon...' : 'Uploading...'}
            </span>
          ) : (
